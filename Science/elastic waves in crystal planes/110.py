...
# Create the wave in [110] direction
def wave_displacement(position, k, omega, t):
    # Wavevector in the [110] direction
    k_vector = k * np.array([1, 1, 0]) / np.sqrt(3)
...
displaced_positions_init = fcc_positions + np.outer(displacements_init, np.array([1, 1, 0]) / np.sqrt(3))
...
arrow_end = np.array([1, 1, 0]) / np.sqrt(3) * arrow_length  # Normalize [111] direction
...
fig.update_layout(
    ...
    title="3D FCC Lattice with Wave Propagation and [110] Direction",
